    ...
    RSGCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == Config.LawJob then
            if badge == false then
                ...
                RSGCore.Functions.Notify(Lang:t('primary.badge_put_on'), 'primary')
                badge = true
            else
                if not IsPedMale(ped) then -- female
                ...
                RSGCore.Functions.Notify(Lang:t('primary.badge_taken_off'), 'primary')
                badge = false
            end
        else
            RSGCore.Functions.Notify(Lang:t('error.only_law_person_can_do_this'), 'error')
        end
    end)
end)
