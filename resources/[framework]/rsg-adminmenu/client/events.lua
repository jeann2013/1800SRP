...
RegisterNetEvent('admin:client:SaveCar', function()
    ...
    if veh ~= nil and veh ~= 0 then
        local plate = RSGCore.Functions.GetPlate(veh)
        local props = RSGCore.Functions.GetVehicleProperties(veh)
        local hash = props.model
        local vehname = getVehicleFromVehList(hash)
        if RSGCore.Shared.Vehicles[vehname] ~= nil and next(RSGCore.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('admin:server:SaveCar', props, RSGCore.Shared.Vehicles[vehname], GetHashKey(veh), plate)
        else
            RSGCore.Functions.Notify(Lang:t("error.no_store_vehicle_garage"), 'error')
        end
    else
...
RegisterNetEvent('qb-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    ...
    if weapon ~= "current" then
        ...
        RSGCore.Functions.Notify(Lang:t("info.ammoforthe", {value = ammo, weapon = RSGCore.Shared.Weapons[weapon]["label"]}), 'success')
    else
        ...
        if weapon ~= nil then
            ...
            RSGCore.Functions.Notify(Lang:t("error.no_weapon"), 'error')
        end
    end
end)
